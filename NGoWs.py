...
from scripts.all_stats import ScrabbleStats
from scripts.checker import ScrabbleChecker
...
class ScrabbleGame:
    ...
    def empty_space_test(self):
        ...
        x_thrities = [221, 313, 405, 497, 589]
        y_thirties = [x-100 for x in x_thrities]

        a = 0
        #for index, id_number in self.index_and_number.items()

        for rect in self.used_rects[1:-1]:
    ...
    def valid_follow_up(self):
        ...
        #Only one collision is necessary, so any True in self.validate_moves makes a play valide.
        if self.moved_rects:
        ...
        for column in self.board_copy:
            ...
            for let in column:
                if not let:
                    ...
                    held_vertical += list(let.keys())[0]
                    vertical_id += list(let.values())[0]


        for n in np.array(range(15)):
            #To scan from left to right, top to bottom
            for row in self.board_copy:
                row.append({})
                if not row[n]:
                    if held_horizontal:
                        #Filter and append
                    ...
                    held_horizontal += list(row[n].keys())[0]
                    horizontal_id += list(row[n].values())[0]
        ...
        if len(self.invalid_words) > 0:
            ...
            if self.valid_words:
                ...
                for l in range(len(self.let.rack_rects)):
                    self._undo(l)
                sfx.invalid_word_sound.play()
                self.stats.news = f"Not valid words"
                self.board.prep_news()
                self.accepted = False
